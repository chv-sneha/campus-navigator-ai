...
import { GraduationCap, Sparkles, BookOpen, Clock } from "lucide-react";
    ...
    <div className="min-h-screen flex">
      ...
      >
        ...
        <div className="relative z-10 max-w-md text-center">
          ...
          <h1 className="font-display text-4xl font-bold text-primary-foreground mb-3">CampusIQ</h1>
          <p className="text-lg mb-12" style={{ color: "hsl(220, 20%, 75%)" }}>
            Your AI co-pilot for college life
          </p>

          <div className="space-y-6 text-left">
            {[
              { icon: Sparkles, title: "AI-Powered Learning", desc: "Get concept explanations in seconds with interactive reels" },
              { icon: BookOpen, title: "Smart Documents", desc: "Generate assignments formatted to your professor's style" },
              { icon: Clock, title: "Never Miss a Deadline", desc: "Track deadlines, goals and daily schedules effortlessly" },
            ].map((f, i) => (
              <div key={i} className="flex items-start gap-4">
                <div className="w-10 h-10 rounded-lg flex items-center justify-center shrink-0" style={{ background: "hsl(220, 72%, 45%, 0.3)" }}>
                  <f.icon className="w-5 h-5 text-accent" />
                </div>
                <div>
                  <h3 className="font-display font-semibold text-primary-foreground">{f.title}</h3>
                  <p className="text-sm" style={{ color: "hsl(220, 20%, 65%)" }}>{f.desc}</p>
                </div>
              </div>
            ))}
          </div>
